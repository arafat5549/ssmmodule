...
import com.ssf.rp.service.IRpAccountService;
...
/**
 * 
 * IRpAccountService 接口实现类
 *
 * @author wang
 */
@Service(value = "rpAccountService")
public class RpAccountServiceImpl implements IRpAccountService {

	private static final Logger LOG = LoggerFactory.getLogger(RpAccountServiceImpl.class);
	private static final String PREFIX_CAHCE = RedisCache.CAHCENAME + "|RpAccount|";
	private static final Class<RpAccount> SELF_CLASS = RpAccount.class;

	@Resource(name = "rpAccountDao")
	...
	@Override
	public List<RpAccount> selectListByMap(Map<Object, Object> map) {
	...
	@Override
	public List<RpAccount> listPage(int offset,int limit,Map<Object, Object> map)
	{
	...
	@Override
	public RpAccount selectByPrimaryKey(String id) {
		String cache_key = PREFIX_CAHCE + "selectByPrimaryKey|" + id;
		if(springContextHelper.getRedisAcaliable())
		{
			RpAccount result_cache = redisCache.getCache(cache_key, SELF_CLASS);
	...
	@Override
	public int insert(RpAccount record) {
		return mapper.insert(record);
	}

	@Override
	public int insertSelective(RpAccount record) {
		return mapper.insertSelective(record);
	}

	@Override
	public int updateByPrimaryKeySelective(RpAccount record) {
		return mapper.updateByPrimaryKeySelective(record);
	}

	@Override
	public int updateByPrimaryKey(RpAccount record) {
		return mapper.updateByPrimaryKey(record);
	}
	
	@Override
	public MyPageView<RpAccount> generateMyPageViewVO(Map<Object, Object> map) {
		
		int _totalCount = mapper.selectCountByMap(map);
		
		List<RpAccount> _list = mapper.selectListByMap(map);
		...
		MyPageView<RpAccount> pageView = MyPageView.generaterMyPageView(recordPerPage, currentPage, _totalCount, _list);
